...
async function main() {
  ...
  app.get("/tournaments", async (req, res) => { 
    ...
    for await (const collection of collection_info_cursor) {
      const info_doc = await db.collection(collection.name).findOne({type: "info"})
      if (info_doc != null) {
        ...
        logger.debug("Get Tournaments - Name: "+tournament_info.name +" and ID: "+tournament_info.id)
      } else {
  ...
  app.post("/tournaments", async (req, res) => {
    ...
    const info_doc = await db.collection(tournament_prefix+id.toString()).insertOne({     
        "type": 'info',
        "name": name,
        "id": id,
        "values": values  
